  ...
  stages {
    ...
    stage('Build/Push Container') {
      when {
        ...
        expression {
          container('ubuntu') {
            sh(returnStatus: true, script: 'skopeo inspect docker://docker.io/robinhoodis/diagrams:`cat VERSION`') == 1
          }
        }
      }
      steps {
        container(name: 'kaniko', shell: '/busybox/sh') {
          script {
            ...
            /kaniko/executor --dockerfile=Dockerfile \
                             --context=`pwd` \
                             --destination=robinhoodis/diagrams:`cat VERSION` \
                             --destination=robinhoodis/diagrams:latest \
                             --cache=true
            '''
          }
        }
