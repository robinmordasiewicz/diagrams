...
cat deployment.yaml | sed -re "s/image:[[:space:]]robinhoodis\/diagrams:.*/image: robinhoodis\/diagrams:${CONTAINERVERSION}/" > deployment.yaml.tmp && mv deployment.yaml.tmp deployment.yaml
